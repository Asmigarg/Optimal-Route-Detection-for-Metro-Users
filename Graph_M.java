...
public class Graph_M {
	...
	public double calculateFare(String source, String destination) {
        HashMap<String, Boolean> processed = new HashMap<>();
        if (!containsVertex(source) || !containsVertex(destination) || !hasPath(source, destination, processed)) {
            return -1; // Invalid input or no path exists
        }

        int distance = dijkstra(source, destination, false);

        // Define fare structure (you can adjust these values as needed)
        double baseFare = 20; // Base fare for first 5 km
        double ratePerKm = 5; // Rate per km after first 5 km
        ...
        if (distance <= 5) {
            ...
            fare = baseFare + (distance - 5) * ratePerKm;
        }
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			System.out.println("4. GET FARE FROM A 'SOURCE' STATION TO 'DESTINATION' STATION");
			...
			switch (choice) {
				...
				case 3:
                    ...
                    System.out.println("ENTER THE SOURCE AND DESTINATION STATIONS");
                    if (ch1 == 1) {
                        st3 = keys1.get(Integer.parseInt(inp.readLine()) - 1);
                        st4 = keys1.get(Integer.parseInt(inp.readLine()) - 1);
                    } else if (ch1 == 2) {
                        String a, b;
                        a = (inp.readLine()).toUpperCase();
                        for (x = 0; x < keys1.size(); x++)
                            if (a.equals(codes1[x]))
                                break;
                        st3 = keys1.get(x);
                        b = (inp.readLine()).toUpperCase();
                    ...
                    double fare = g.calculateFare(st3, st4);
                    if (fare == -1) {
                        System.out.println("THE INPUTS ARE INVALID OR NO PATH EXISTS");
                    } else {
                        System.out.printf("FARE FROM %s TO %s IS Rs. %.2f\n", st3, st4, fare);
                    }
					break;
